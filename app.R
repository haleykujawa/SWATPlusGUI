                   ...
                   tabsetPanel(
                     tabPanel("Management and conservation practices",br(),
                              tags$div(tags$p('Total rate of all seven management scenarios needs to add up to 100%'),
                                ...
                                tags$p('To calculate an even rate on a subset of scenarios for grassed waterways (grww) or vegetated buffers, use the following equation:'),
                                tags$p('Input rate = desired total rate of buffers or grww on cropland (percent) (1 / % of total cropland to have buffer/grww on (decimal percent)) ')),
                              br(),
                   #management widgets       
...
#from https://www.r-bloggers.com/2014/04/deploying-desktop-apps-with-r/
  ...
  output$buff_grw_rate<-reactive({validate(need(( (input$CSFT_B + input$CSFT_GW) <= 100) & ((input$CSNT_B + input$CSNT_GW) <= 100) & ((input$CSRT_B + input$CSRT_GW) <= 100) & ((input$CSRot_B + input$CSRot_GW) <= 100) & ((input$CSNTcc_B + input$CSNTcc_GW) <= 100) & ((input$CSWS_B + input$CSWS_GW) <= 100), "Total rate of buffers and grassed waterways on one management scenario cannot be greater than 100% -- Adjust before running SWAT+"))
    paste0("Total input rate of vegetated buffers is ", round(input$CSFT*input$CSFT_B/100 +input$CSNT*input$CSNT_B/100+ input$CSRT*input$CSRT_B/100+ input$CSRot*input$CSRot_B/100+ input$CSNTcc*input$CSNTcc_B/100 +input$CSWS*input$CSWS_B/100 +input$CSWcc*input$CSWcc_B/100), '%',
           " and total input rate of grassed waterways is ", round(input$CSFT*input$CSFT_GW/100 +input$CSNT*input$CSNT_GW/100+ input$CSRT*input$CSRT_GW/100+ input$CSRot*input$CSRot_GW/100+ input$CSNTcc*input$CSNTcc_GW/100 +input$CSWS*input$CSWS_GW/100 +input$CSWcc*input$CSWcc_GW/100), '%')})
...
###################################################
  ...
  text_reactive <-eventReactive( input$runswat, {
   ...
   showModal(modalDialog("Running SWAT+ .. any further changes will not be reflected in results tab", footer=NULL,easyClose = T))
